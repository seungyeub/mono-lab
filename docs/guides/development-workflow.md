...
CI의 VRT는 Playwright 공식 컨테이너(`mcr.microsoft.com/playwright`) 안에서 실행되므로, **PR을 통과시키는 기준은 컨테이너 렌더링**입니다. 대부분의 스냅샷은 macOS 로컬 촬영본이 5% 허용치 안에 들어 그대로 커밋해도 되지만, 텍스트 밀도가 높은 페이지(예: `project-detail`의 Desktop Chrome)는 폰트 렌더링 차이가 허용치를 넘을 수 있습니다.
...
- **컨테이너의 WebKit은 실제 Safari와 다르게 그릴 수 있습니다.** Hero의 모바일 기준 이미지에는 3D 카드 사진 이마 위에 검은 사각형이 찍혀 있는데, 2026-09-25 카드 모델을 바꾼 뒤부터 생긴 것입니다. 모델 안의 텍스처 이미지는 깨끗하고, 로컬 Chrome과 실제 iPhone에서는 보이지 않았습니다. 컨테이너에서는 매번 같게 그려져 VRT는 안정적으로 통과하므로 그대로 둡니다 - **이 사각형을 결함으로 보고 고치려 하지 마세요.** 카드 모델을 다시 바꾸면 이 줄도 확인합니다.
