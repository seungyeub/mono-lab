...
- **시각적 회귀 테스트(Visual Regression Testing) 점진적 스케일업:**
  ...
  1. **[Intermediate] Docker 기반 로컬 테스트 환경:** 만약 렌더링 오차가 허용치를 넘어 CI 실패 빈도가 잦아진다면, 로컬 환경에서도 Docker 컨테이너를 띄워 CI(Linux)와 100% 동일한 렌더링 환경에서 스냅샷을 갱신하도록 아키텍처를 개선하여 오차를 완벽히 통제합니다.
