...
order: 7
image: '/images/projects/ppcwiz.webp'
carouselImages:
  - '/images/projects/ppcwiz.webp'
  - '/images/projects/ppcwiz-02.webp'
...
overview:
  ...
  - title: '규모'
    description: 'package.json만 있는 빈 저장소에서 webpack/babel 빌드 환경부터 직접 구축해 30여 개 화면·667개 파일 규모의 SPA를 단독 구축.'
implementation:
  codeLanguage: 'javascript'
  codeCaption: '공통 생명주기는 강제하고 모듈별 예외만 훅으로 열어둔 비동기 액션 규약 (디버그 로그 1줄 생략)'
  codeSnippet: |
    /** pending */
...
PPCWIZ는 아마존 셀러의 스폰서드 광고를 규칙 기반으로 분석하고 입찰가를 자동 조정하는 SaaS다. package.json만 있는 빈 저장소에서 webpack/babel 빌드 환경부터 직접 구축해 SPA(wiz-view)를 처음부터 만들었다. 디자인 시스템이 따로 없어 UI/UX 디자인도 Sketch로 직접 수행했다.
