 ...
 * 홈 FAQ 섹션의 문답 데이터.
 *
 * FAQSection이 클라이언트 컴포넌트라 그 안에 두면 서버에서 읽을 수 없다.
 * 같은 문답을 JSON-LD(FAQPage)로도 내보내야 하므로 데이터만 분리했다 —
 * 화면과 구조화 데이터가 서로 다른 내용을 말하는 것을 막는 것이 목적이다.
 *
 * 주석 처리된 항목은 노출하지 않기로 한 문답이다. 되살릴 때 화면과 JSON-LD가
 * 함께 갱신되도록 여기 그대로 남겨 둔다.
 */
